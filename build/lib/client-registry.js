...
class ClientRegistry {
    ...
    async restore() {
        ...
        try {
            channels =
                (_a = await this.adapter.getForeignObjectsAsync(`${this.adapter.namespace}.clients.*`, 'channel')) !=
                null
                    ? _a
                    : {};
        } catch (err) {
    ...
    async handleManualUrlWrite(id, rawValue) {
        ...
        if (!result.ok) {
            ...
            await this.adapter.setStateAsync(`clients.${id}.manualUrl`, {
                val: (_a = record.manualUrl) != null ? _a : '',
                ack: true,
            });
            return;
        }
    ...
    async createClient(ip, hostname) {
        ...
        this.adapter.log.info(
            `New client registered: ${id}${ip ? ` (${hostname != null ? hostname : ip})` : ''}, mode='${mode}'`,
        );
        return record;
    }
    /**

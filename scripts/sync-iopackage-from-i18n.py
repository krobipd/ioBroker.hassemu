...
Hintergrund: bis v1.32.0 wurden `common.name`/`common.desc` der instanceObjects
parallel in zwei Quellen gepflegt (io-package.json + i18n-states.ts). Drift
zwischen den zwei Stellen war B1+B2-Vorfall (truncated strings in 11 Sprachen).
v1.32.0 hatte einen read-only Drift-Check (check-iopackage-i18n-sync.py); ab
v1.32.2 ist das auto-correct: i18n-states.ts ist single-source-of-truth, die
common.name/desc-Blocks in io-package.json werden bei jedem Release neu aus
i18n-states.ts gerendert.

Drift wird nicht mehr nur gemeldet — sie wird gefixt. Beim Release-Commit landet
das synchronisierte io-package.json automatisch.
...
def main() -> int:
    ...
    desc_changes, desc_errors = sync_field(instance_objects, state_descs, DESC_MAPPING, "desc")
